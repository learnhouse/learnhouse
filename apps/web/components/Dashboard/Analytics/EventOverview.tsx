...
function ChartTooltip({ active, payload, label, breakdownMap }: any) {
          ...
          {bd.top_referrer?.length > 0 && bd.top_referrer[0] && (
            <div className="border-t border-gray-100 pt-2 mt-2">
              <div className="flex items-center gap-1.5 text-[11px]">
                ...
                <span className="text-gray-400">Top Referrer</span>
...
const CARD_MODAL_MAP: Record<string, string> = {
  Signups: 'signups',
  Enrollments: 'enrollments',
  Completions: 'completions',
  // Submissions: no detail query yet
}
...
export default function EventOverview({ days = '30' }: { days?: string }) {
  ...
  // Index breakdown data by date for tooltip lookup
  ...
  const cards = Object.entries(EVENT_META).map(([event, meta]) => {
    ...
    return { ...meta, total: row?.total ?? 0, unique: row?.unique_users ?? 0 }
  })
  ...
  return (
    ...
    <div className="bg-white nice-shadow rounded-xl overflow-hidden">
      ...
      <div className="grid grid-cols-5 divide-x divide-gray-100">
        ...
        >
          <ArrowsOutSimple size={14} className="absolute top-3 right-3 text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity" />
          <div className="flex items-center gap-1.5 mb-1">
            <Broadcast size={14} weight="fill" className="text-green-500" />
            <span className="text-xs font-medium text-gray-400">Live Now</span>
          </div>
          <div className="text-2xl font-bold text-gray-900">
            {liveLoading ? '—' : liveCount.toLocaleString()}
          </div>
          <p className="text-[11px] text-gray-300 mt-0.5">Last 5 minutes</p>
        </div>
        {/* Event stat cells */}
        {cards.map((card) => {
          const modalKey = CARD_MODAL_MAP[card.label]
          return (
            <div
              key={card.label}
              className={`px-5 py-4 ${modalKey ? 'cursor-pointer hover:bg-gray-50 transition-colors group relative' : ''}`}
              onClick={modalKey ? () => setActiveModal(modalKey) : undefined}
            >
              {modalKey && <ArrowsOutSimple size={14} className="absolute top-3 right-3 text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity" />}
              <div className="flex items-center gap-1.5 mb-1">
                {card.icon}
        ...
        {dauLoading ? (
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={dauRows}>
              ...
              />
              <Tooltip
                content={<ChartTooltip breakdownMap={breakdownMap} />}
      ...
      {/* Visitor insights row: Country | Device | Referrer */}
      <div className="border-t border-gray-100 grid grid-cols-3 divide-x divide-gray-100">
        {/* Countries */}
        <div className="p-5">
          <div className="flex items-center gap-2 mb-3">
            <GlobeHemisphereWest size={15} weight="duotone" className="text-blue-400" />
            <h3 className="text-sm font-semibold text-gray-700">Countries</h3>
          </div>
          {countryLoading ? (
            <div className="h-[160px] flex items-center justify-center text-gray-300">Loading...</div>
          ) : countryRows.length === 0 ? (
            <div className="h-[160px] flex items-center justify-center text-gray-300 text-xs">No data yet</div>
          ) : (
        ...
        <div className="p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-gray-700">Top Referrers</h3>
          </div>
          {referrerLoading ? (
            <div className="h-[160px] flex items-center justify-center text-gray-300">Loading...</div>
      ...
      <AnalyticsDetailModal
        ...
        onOpenChange={(open) => !open && setActiveModal(null)}
        title="Signups"
        description={`New user registrations in the last ${days} days`}
      ...
      <AnalyticsDetailModal
        ...
        onOpenChange={(open) => !open && setActiveModal(null)}
        title="Enrollments"
        description={`Course enrollments in the last ${days} days`}
      ...
      <AnalyticsDetailModal
        ...
        onOpenChange={(open) => !open && setActiveModal(null)}
        title="Completions"
        description={`Course completions in the last ${days} days`}
