...
interface MagicBlockModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (_htmlContent: string, _sessionUuid: string) => void
  blockUuid: string
  activityUuid: string
...
function MagicBlockModal({
  ...
  const handleSendMessage = async (message: string) => {
    ...
    track(AnalyticsEvent.MagicBlockGenerationRequested, {
      iteration_count: revisions.length,
  ...
  const handleSave = () => {
    if (htmlContent && sessionUuid) {
      track(AnalyticsEvent.MagicBlockSaved, { iteration_count: revisions.length })
