...
function parseActivityPath(pathname: string | null) {
  ...
  return { courseUuid: cleanUuid(match[1]), activityUuid: cleanUuid(match[2]) }
}

function findModuleName(course: any, activityUuid: string) {
  const clean = cleanUuid(activityUuid)
  for (const chapter of course?.chapters || []) {
    const found = (chapter.activities || []).some(
      (activity: any) => cleanUuid(activity.activity_uuid) === clean
    )
...
function looksLikeForwardNavigation(target: EventTarget | null) {
  if (!(target instanceof Element)) return false
  const anchor = target.closest('a[href]') as HTMLAnchorElement | null
  if (anchor?.href && (/\/activity\//.test(anchor.href) || /\/course\//.test(anchor.href))) return true
...
export default function AppliedLearningGate() {
  ...
  const reflectionQuery = useQuery({
    ...
    enabled: !!activityUuid && session?.status === 'authenticated',
    staleTime: 30_000,
  })
  ...
  const routeRef = useRef<{ pathname: string; activityUuid: string; satisfied: boolean } | null>(null)
  ...
  // Fallback route guard. It catches navigation triggered programmatically by
  // the existing completion control, then returns the learner to the lesson if
  // they tried to leave without recording application.
  useEffect(() => {
    if (!pathname) return
    const previous = routeRef.current
    if (
      previous &&
      previous.activityUuid &&
      previous.pathname !== pathname &&
      !previous.satisfied
    ) {
      sessionStorage.setItem('acyber-apply-open', previous.activityUuid)
      router.replace(previous.pathname)
      return
    }
    ...
    const requested = sessionStorage.getItem('acyber-apply-open')
    if (requested && requested === activityUuid) {
      sessionStorage.removeItem('acyber-apply-open')
      setOpen(true)
    }
  }, [pathname, activityUuid, router])

  // Prevent ordinary links and buttons from leaving the activity while the
  // reflection is missing. The fallback route guard above covers any control
  // that navigates entirely in code.
  useEffect(() => {
    if (!activityUuid || reflectionQuery.isLoading || satisfied) return
    const handleClick = (event: MouseEvent) => {
      if (!looksLikeForwardNavigation(event.target)) return
  ...
  }, [activityUuid, reflectionQuery.isLoading, satisfied])
  ...
  const save = async () => {
    if (plannedApplication.trim().length < 3) {
      toast.error('Tell us how you will apply this learning before continuing.')
      return
    }
    setSaving(true)
    try {
      const saved = await saveAppliedLearningReflection(
        {
          org_id: org.id,
          course_uuid: course?.course_uuid || `course_${courseUuid}`,
          activity_uuid: activity?.activity_uuid || `activity_${activityUuid}`,
          activity_name: activity?.name || '',
          module_name: moduleName,
          planned_application: plannedApplication,
          previous_application: previousApplication,
          measurable_change: measurableChange,
      ...
      if (routeRef.current) routeRef.current.satisfied = true
      setOpen(false)
      toast.success('Saved to your applied learning portfolio.')
    } catch (error: any) {
      toast.error(error?.message || 'Could not save your application note. Please try again.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <>
      ...
      >
        ...
        {satisfied ? 'Applied learning saved' : 'Apply this at work'}
      </button>
        ...
        <div className="fixed inset-0 z-[1000] flex items-end justify-center bg-black/50 p-0 backdrop-blur-[2px] sm:items-center sm:p-5">
          <div className="max-h-[94vh] w-full overflow-y-auto rounded-t-[28px] bg-white shadow-2xl sm:max-w-2xl sm:rounded-[28px]">
            <div className="sticky top-0 z-10 flex items-start justify-between border-b border-black/[0.07] bg-white px-5 py-5 sm:px-7">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em]" style={{ color: BRAND_RED }}>Before you continue</p>
                <h2 className="mt-1 text-2xl font-black tracking-[-0.035em] text-[#101418]">Apply what you just learned.</h2>
                <p className="mt-1 text-sm text-black/50">This becomes part of your lifetime learning portfolio.</p>
              </div>
              <button onClick={() => setOpen(false)} className="rounded-full p-2 text-black/40 hover:bg-black/[0.05]" aria-label="Close">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-6 px-5 py-6 sm:px-7">
              <div>
                ...
                <textarea
                  value={plannedApplication}
                  onChange={(e) => setPlannedApplication(e.target.value)}
                  rows={4}
                  placeholder="For example: In our next management meeting I will use this approach to..."
                  className="mt-3 w-full resize-y rounded-2xl border border-black/10 bg-[#FAFAFA] p-4 text-sm leading-6 outline-none transition focus:border-[#C51635]/50 focus:ring-4 focus:ring-[#C51635]/[0.07]"
                />
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-black/45">Optional. Record what you actually did, not what you intended to do.</p>
                <textarea
                  value={previousApplication}
                  onChange={(e) => setPreviousApplication(e.target.value)}
                  rows={3}
                  placeholder="I applied..."
                  className="mt-3 w-full resize-y rounded-2xl border border-black/10 bg-[#FAFAFA] p-4 text-sm leading-6 outline-none transition focus:border-[#C51635]/50 focus:ring-4 focus:ring-[#C51635]/[0.07]"
                />
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-black/45">Optional today. Come back and update it when the result is visible.</p>
                <textarea
                  value={measurableChange}
                  onChange={(e) => setMeasurableChange(e.target.value)}
                  rows={3}
                  placeholder="Time saved, revenue gained, errors reduced, response time improved, people reached, decision quality improved..."
                  className="mt-3 w-full resize-y rounded-2xl border border-black/10 bg-[#FAFAFA] p-4 text-sm leading-6 outline-none transition focus:border-[#C51635]/50 focus:ring-4 focus:ring-[#C51635]/[0.07]"
                />
              </div>
              ...
              <details className="rounded-2xl border border-black/[0.07] bg-white px-4 py-3">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Link, document name, metric source or other evidence..."
                  className="mt-3 w-full resize-y rounded-xl border border-black/10 bg-[#FAFAFA] p-3 text-sm leading-6 outline-none"
                />
              </details>
            </div>
            ...
            <div className="sticky bottom-0 flex flex-col gap-3 border-t border-black/[0.07] bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
              <p className="text-xs leading-5 text-black/45">You can update outcomes later as the measurable change becomes clear.</p>
              <button
                type="button"
                onClick={save}
                disabled={saving}
                className="min-h-12 shrink-0 rounded-xl px-6 py-3 text-sm font-extrabold text-white disabled:opacity-60"
