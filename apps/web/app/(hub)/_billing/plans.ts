 ...
 * (legacy) ones. Reading the wrong key made the "Pro" badge disagree with the
 * backend-resolved usage limits (badge Pro, limits Free). Resolve it once, here.
 */
export function resolvePlanIdFromOrg(org: any): string {
  const config = org?.config?.config;
  const isV2 =
    typeof config?.config_version === "string" &&
    config.config_version.startsWith("2");
