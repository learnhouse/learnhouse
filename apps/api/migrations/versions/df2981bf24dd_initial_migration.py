...
def upgrade() -> None:
    ...
    op.create_foreign_key(
        "trail_user_id_fkey", "trail", "user", ["user_id"], ["id"], ondelete="CASCADE"
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint("trail_org_id_fkey", "trail", type_="foreignkey")
    op.create_foreign_key(
        "trail_org_id_fkey", "trail", "organization", ["org_id"], ["id"]
    )
    op.drop_constraint("trail_user_id_fkey", "trail", type_="foreignkey")
    ...
    op.add_column(
        ...
        sa.Column(
            "assignment_user_uuid", sa.VARCHAR(), autoincrement=False, nullable=False
        ),
    )
