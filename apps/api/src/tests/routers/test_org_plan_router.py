...
async def test_update_plan_invalidates_caches(client, db):
    # A plan change must proactively clear the config, org-by-slug and usage
    # caches so the new plan/limits are reflected immediately (not after TTL).
    await _add_org(db, config={"config_version": "1.0"})
    with patch(
        "src.routers.orgs.org_plan.update_org_with_config_no_auth", new=AsyncMock()
    ), patch("src.routers.orgs.org_plan.invalidate_org_config_cache") as m_cfg, patch(
        "src.routers.orgs.org_plan.invalidate_org_cache"
    ) as m_slug, patch(
        "src.routers.orgs.org_plan.invalidate_usage_cache"
    ) as m_usage:
        res = await client.put(PATH, headers=HDR, json={"org_id": 1, "plan": "pro"})

    assert res.status_code == 200
    m_cfg.assert_called_once_with(1)
    m_usage.assert_called_once_with(1)
