"""Markdown → Tiptap JSON (apply-path).

Duplicated from `apps/mcp/src/learnhouse_mcp/markdown.py` so the API can
materialize pending edits without a round-trip back to the MCP server.
Keep the two in sync — same node/mark set, same fallback behaviour.
"""
...
def markdown_to_tiptap(md: str) -> TiptapNode:
    ...
    try:
        ...
        raise MarkdownConversionError(f"Could not parse markdown: {exc}") from exc
...
def _convert_block_tokens(
    ...
    while i < end:
        ...
        if ttype == "paragraph_open":
            ...
            i += 3
            content = _convert_inline(inline.children or [])
            out.append({"type": "paragraph", "content": content} if content else {"type": "paragraph"})
            continue
