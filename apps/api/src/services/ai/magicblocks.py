...
async def generate_magicblock_stream(
    ...
    try:
        ...
        # Prior conversation turns. If the user pasted a style reference, prime the
        # model with it as a design exemplar BEFORE the actual request so the new
        # block inherits its look.
        history: list[dict] = []
        if style_reference:
...
Acknowledge the style and wait for the user's actual request."""
            history.append({"role": "user", "content": style_prompt})
            history.append({"role": "model", "content": "Understood. I'll match the design language of the reference block — palette, typography, spacing, component shapes, and library choices — while generating entirely new content for the user's request."})
        ...
        if current_html and session.iteration_count > 0:
            user_prompt = f"""You are editing an existing interactive element. The user wants a SURGICAL CHANGE, not a redesign.
