...
| `npx learnhouse update` | Update to latest or a specific version |
| `npx learnhouse status` | Show service status |
| `npx learnhouse health` | Run health checks |
| `npx learnhouse logs` | Stream container logs |
| `npx learnhouse config` | Show installation info |
| `npx learnhouse env` | Edit environment variables |
| `npx learnhouse backup` | Backup database |
| `npx learnhouse restore <archive>` | Restore database from backup |
| `npx learnhouse doctor` | Run diagnostics |
| `npx learnhouse deployments` | Manage deployments & resource limits |
| `npx learnhouse shell` | Open shell in a container |
