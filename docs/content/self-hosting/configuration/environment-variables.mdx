...
| `LEARNHOUSE_SELF_HOSTED` | Legacy self-hosting flag used in tenancy inference. Prefer `LEARNHOUSE_TENANCY`. | boolean | `false` | API |
| `LEARNHOUSE_USE_DEFAULT_ORG` | Legacy default-org flag used in tenancy inference. Prefer `LEARNHOUSE_TENANCY`. | boolean | `false` | API |
...
| `LEARNHOUSE_PLATFORM_URL` | Base URL for LearnHouse platform links, upgrade URLs, and email allowlist checks. | URL | unset; some platform flows fall back to `https://learnhouse.app` | API, Web |
...
| Variable | Description | Type | Default / required | Used by |
| --- | --- | --- | --- | --- |
| `LEARNHOUSE_LICENSE_KEY` | Enterprise license key. | secret string | required for EE installs and updates | EE CLI, Web EE messaging |
| `LEARNHOUSE_LICENSE_SERVER_URL` | Enterprise license server URL. | URL | `https://partners.learnhouse.app` | EE CLI |
